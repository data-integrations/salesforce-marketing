...
public class MarketingConnectorConfig extends PluginConfig {
  ...
  @Macro
  @Description("OAuth2 client ID associated with an installed package in Salesforce Marketing Cloud.")
  private final String clientId;

  @Name(MarketingCloudConstants.PROPERTY_CLIENT_SECRET)
  @Macro
  @Description("OAuth2 client secret associated with an installed package in Salesforce Marketing Cloud.")
  ...
  public void validateSalesforceConnection(FailureCollector collector) {
    try {
      MarketingCloudClient.getOrCreate(getClientId(), getClientSecret(), getAuthEndpoint(), getSoapEndpoint());
  ...
  /**
   * Returns true if any of the required parameters are macro enabled
   */
  ...
  public Schema getSchema(SourceObject sourceObject) throws ETSdkException {
    return MarketingCloudInputFormat.getTableMetaData(sourceObject, null, MarketingCloudClient.getOrCreate
      (getClientId(), getClientSecret(), getAuthEndpoint(), getSoapEndpoint())).getSchema();
  }
}
